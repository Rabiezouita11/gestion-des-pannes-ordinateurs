  ...
  @foreach ($enseignant as $l)
              <tr>
                  ...
                  <td><img src="{{asset('storage/'.$l->image)}}" alt=" " class="img-fluid"  width="100px" height="100px"></td>
             
                  <td>
                <a  href="{{ route('SupprimerEnseignant', ['id' => $l->id]) }}" style="color:red" class="far fa-trash-alt"  onclick="return confirm('Etes-vous sur ?');"></a>
...
@if (count($errors) >0)
                        ...
                        @endif
                        @if (request()->input())
<h6>{{$enseignant->total()}} resultat(s) pour la recherche "{{request()->r}}"</h6>
@endif
<br>
